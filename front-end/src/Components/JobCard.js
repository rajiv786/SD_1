...
const JobCard = ({ job }) => {
  ...
  return (
    <Grid item xs={12} sm={6} md={4} lg={4}> {/* Adjust the grid sizes based on your layout */}
      <Card sx={{ maxWidth: 300, margin: 2,background:'peachpuff',borderRadius:'20px'}}>
        <CardContent>
          ...
          <Button color="primary" variant="contained" onClick={() => window.open(job.jdLink, "_blank")}>Apply</Button>
        </CardContent>
      </Card>
    </Grid>
  );
